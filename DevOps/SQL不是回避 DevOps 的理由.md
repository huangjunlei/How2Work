...
一旦你的软件使用了这些技术，采用 CI（持续集成）会变得非常容易。你的自动化测试环境可以包含测试使用旧的 schema 的数据库并升级它，然后运行新版软件。你的 schema 升级过程可以在发布到生产环境前，进行数百次的测试。这会为该过程带来更多的信心，减少schema升级的危险，解藕DBA个人参与到升级。他们可以享受原本属于他们的周末了。

我对此技术最感兴趣的是，你的schema可以按代码一起管理了。大幅减少了控制台上的手工操作，可以在开发环境、测试环境、UAT（用户验收厕所）环境、生产环境中不断演练整个过程。你可以重复执行这个过程，完善它。既然它是代码，你可以应用代码管理的实践和软件工程的计算去管理它。