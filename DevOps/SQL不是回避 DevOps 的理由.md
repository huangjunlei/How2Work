...
遗憾的是，ACM有套策略来应对现实法则，同样大部分企业雇主也类似。这也是为什么传统方法是关闭整个应用，升级所有东西，然后再重新上线。这种最佳实践延续直至IEEE 的核算了如何暂停的过程。

无论是违法现实规律还是计划的停机，每次停机都会引入更大的问题：你让独立系统发生了变更，但不确定系统重新运行后它们是否都还正常。同样也不知道这些累计的改变会引发什么样的破坏。
...
添加字段也会变得简单，即在第一个实例部署前在 schema 中添加新字段即可。如前所述，我们擦用技术1（应用管理自己的 schema），部署一个新发布，它会修改 schema，但不会使用这个字段。由于合适的事务锁会控制住并行处理，第一个新升级的实例重启后会更新 schema。如果有问题，则金丝雀会宕掉。你就可以修复软件，然后实验新的金丝雀。而回退 schema 的变更也是一个可选的方案。

由于 schema 和软件结构，开发者可以按需启动新字段。过去的升级需要考虑多个团队的需求后才能确定一个维护窗口，而现在过程解耦，所有相关方可以有序的工作，而不必相互锁定。
